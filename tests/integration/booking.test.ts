...
describe('PUT /booking/:bookingId', () => {
  ...
  describe('When token is valid', () => {
    ...
    describe('When body is valid', () => {
      it('Should respond with status 403 if the user do not have a booking', async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
      ...
      it('Should respond with status 404 if the room do not exist', async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
        const hotel = await createHotel();
        const room = await createRooms(hotel);
      ...
      it('Should respond with status 403 if the room do not have vacancy', async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
        const hotel = await createHotel();
        const room = await createRooms(hotel);
        ...
        const newRoom = await createRooms(hotel, 1);
        const newBooking = await createBooking(newUser, newRoom);
